    ...
    module topModule(
            ...
            output PWMOut, PWMClock,           //PWM output
...
// -----------------------    PID error calculation -------------------------------------------
            ...
            // pwm pwm(.clk(clk), .pwm_in(controlOut), .pwm_out(PWMModulation));
             ...
             reg [15:0] pwmTest; 
            always @(posedge clk) begin
                    pwmTest <= data_out_adc_top/260;
             end
              pwm8bits pwm8(.clk100k(clk), .pwm_in(pwmTest), .pwm_out(PWMModulation)); //PWM operating with 8 bits resolution 
            
            //PWM outputs assignment
            assign PWMClock=PWMModulation;  
            
//           always @(posedge clk) begin
//                      PWMClock=PWMModulation; 
//             end  
